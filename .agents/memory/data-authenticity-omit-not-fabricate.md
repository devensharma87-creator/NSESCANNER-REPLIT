...
**Remaining (sequenced) backlog** lives in repo-root `HOME_PORTFOLIO_DATA_AUDIT.md`: index analytics Kite-candle facade + honest labelling (`indicesBoard.ts buildItem`); optional diagnostics endpoints + frontend secondary/delayed/stale badges.

**Non-curated ETF coverage (portfolio):** Two-part fix, both needed. (1) Client `classifyInstrument` must catch ETF tokens fused into the symbol with NO word boundary (CPSEETF, SETFGOLD) — `\bETF\b` alone misses them and they fall through as "Equity". (2) A symbol can be a real tradable ETF with NO detectable token at all (MON100, HDFCSILVER); the only honest way to price these is a last-resort call to the backend `/api/etf/:symbol/quote`, which authoritatively gates recognition (curated whitelist + live Kite instrument master) and 404s/nulls for non-ETFs (verified: RELIANCE 404s). **Why it's safe to "blind-call":** the gate lives server-side, so a blanket last-resort etfQuote for any still-unresolved holding cannot mis-price a plain equity. Guard it with an `etfTried` flag so the dedicated ETF-class branch and the last-resort don't double-call.
