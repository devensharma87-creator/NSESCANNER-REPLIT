...
- **M2 — Drift reconciliation** (~1–2 sessions) — origin-story inventory re-verified against post-recovery reality; `drizzle-kit push` diff = ZERO; deployment-env audit; **closing sub-step: ENV-ISOLATION** (nsescanner_test + .env.test, retire `!includes("dummy")`). DRIFT-P0 lifts.
  ...
  * **30-session clock pre-conditions (all four must be met before clock starts)**: (1) FNO incident resolved — earliest direct balance mutation identified or owner-approved adjustment event documented; (2) account reconciles — `reconcilePaperAccount("FNO")` returns `reconciled=true`; (3) reconciliation gate behavioral tests pass — `ledgerReconciliationGate.behavioral.test.ts` (13 tests, added 2026-07-20) green; (4) SILENT_DRIFT evidence-loss incident resolved — owner confirms the 6 deleted rows were test artifacts OR acknowledges irrecoverable production-evidence loss (see `memory/AUDIT_EVIDENCE_LOSS_2026-07-20.md`).
  * **Persistent-worker hosting prerequisite (roadmap note, added 2026-07-20)**: a sleeping Replit web workspace cannot provide reliable intraday scheduling — the Jul 15–17 signal gap (zero `fno_signal_reasoning` rows across 3 trading sessions) is direct evidence. Sessions where the scheduler was not running MUST NOT be counted toward the 30-session clean window; counting them would inflate the drift-free count. Persistent-worker hosting (Replit autoscale deployment, always-on deploy, or an external cron/worker process) is required **before** the M2c 30-session clock can begin. Add as M2 pre-requisite alongside FNO incident resolution.
