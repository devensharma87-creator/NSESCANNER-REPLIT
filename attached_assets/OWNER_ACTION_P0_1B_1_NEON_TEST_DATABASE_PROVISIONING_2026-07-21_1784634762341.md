...
-- The new project also contains Neon's default database. Remove PUBLIC
-- CONNECT there so the restricted runner cannot reach a second database.
-- The project owner retains access through ownership/explicit privileges.
...
SELECT
  has_database_privilege(
    ...
    'nsescanner_test',
    'CONNECT'
  ) AS can_connect_target,
  has_database_privilege(
    'nsescanner_test_runner',
    'neondb',
    'CONNECT'
  ) AS can_connect_default_database,
...
The privilege-isolation query must return:

- `can_connect_target = true`
- `can_connect_default_database = false`
...
- Runner can connect to nsescanner_test only: YES
- Runner cannot connect to default neondb: YES
- Runner has no USAGE/CREATE on public schema: YES
