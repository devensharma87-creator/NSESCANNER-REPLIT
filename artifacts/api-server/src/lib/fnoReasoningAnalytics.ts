...
export interface ReasoningAnalytics {
  ...
  // T1→stop reversal proxy (no signal-id linker, so group on
  // signalDate+index+setup+direction+strike). Counts groups where BOTH
  // CLOSED_TARGET1 and CLOSED_STOPPED rows exist.
  t1ThenStoppedGroups: number;
  /**
   * Metadata so operators cannot misread `t1ThenStoppedGroups` as an exact
   * trade-level reversal rate (architect note 2026-05-17). The
   * fno_signal_reasoning rows do not carry a signal-id linker; this is a
   * best-effort group-by approximation.
   */
  t1ThenStoppedMeta: {
    proxyMethod: string;
    limitation: string;
...
export function computeReasoningAnalytics(rows: FnoSignalReasoningRow[]): ReasoningAnalytics {
  ...
  const t1ThenStoppedMeta = {
    proxyMethod: "group_by(signalDate,index,setup,direction,strike)",
    limitation:
      "No signal-id linker is persisted on fno_signal_reasoning rows; this " +
      "counts groups where BOTH a CLOSED_TARGET1 and a CLOSED_STOPPED row " +
      "exist for the same (date,index,setup,direction,strike) tuple. It is " +
      "an approximation, not exact trade-level reversal rate.",
  };
  ...
  // T1→stop reversal grouping
  const groupKeyOf = (r: FnoSignalReasoningRow): string =>
    `${r.signalDate}|${r.indexSymbol}|${r.setupKey ?? ""}|${r.direction ?? ""}|${r.selectedStrike ?? ""}`;
  const t1Groups = new Set<string>();
  const stopGroups = new Set<string>();
  ...
  // T1→stop reversal: groups in BOTH sets
  let t1ThenStoppedGroups = 0;
  for (const g of t1Groups) if (stopGroups.has(g)) t1ThenStoppedGroups += 1;
