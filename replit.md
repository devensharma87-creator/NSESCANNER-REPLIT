...
- **Yahoo `yahooTickerFor`**: Supports Yahoo futures and FX symbols without `.NS` suffix. For already-qualified non-Indian tickers (US ADRs like `HDB`/`MMYT`/`IBN`, Shanghai `000001.SS`, dollar index `DX-Y.NYB`), call `fetchChartRaw()` instead of `fetchChart()` — `fetchChart` will blindly append `.NS` and Yahoo will reject the symbol as delisted. `indicesBoard` uses `fetchChartRaw` for any `cfg.category !== "INDIA"` row.
- **Kite F&O instruments — NFO + BFO**: `kiteOptionChain.loadInstruments()` and `oiLab.getDynamicFnoUniverse() / getOiHeatmap()` fetch both NFO (NSE F&O) **and** BFO (BSE F&O — SENSEX/BANKEX index futures and options) in parallel, treating either fetch failing as an empty list so a missing BSE-segment entitlement doesn't take down the whole chain. Quote keys are built as `${leg.exchange}:${leg.tradingsymbol}` (not hard-coded `NFO:`) so BFO-served legs are quoted correctly. Without this the SENSEX/BANKEX option chains were silently empty even with a valid Kite session.
- **Kite ticker self-restart on `noreconnect`**: `kiteFeed.startTicker()` schedules an exponential-backoff restart loop (60s → 600s cap) when Kite's built-in autoReconnect (50×5s ≈ 4 min) gives up. Each restart attempt re-checks `getActiveSession()` and bails out cleanly if the session is gone (so a 06:00 IST token expiry doesn't trigger an infinite restart storm). Backoff is reset on every successful start. Disconnect/error handlers use `describeWsError()` which extracts `code/type/reason/message` instead of producing log lines like `err="[object Object]"`.
