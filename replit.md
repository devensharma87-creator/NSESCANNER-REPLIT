...
- **Outstanding audit backlog (next session)**: (1) Blank-panel empty-state copy on `deep-scan`, `flows`, `options` charts (T009 — needs fresh-eyes per-panel investigation, generic "no data" cards still appear when upstream returns []). (2) Indices Board "Loading…" stuck-state server-side diagnosis (T008 server-side; UI-side guards already shipped on `market-mood.tsx` AND `mmi-gauge.tsx`). (3) Yahoo-fallback gating for F&O Top-50 (T012, server-side). (4) 15-second opening-window polling for F&O Top-50 (T013). (5) FVG / Liquidity Sweep / Volume-Δ engines (T014–T016, blocked by T012). All five SMC items must ship behind feature flags so any single one can be disabled by env var. **Shipped this batch (T001-T011)**: `/legal/*` chrome strip in `layout.tsx`; `mmi-gauge.tsx` parity guard with `market-mood.tsx`; `DataSourceBadge` on dashboard / sectors / news / deep-scan / option-chain (uses `chain.generatedAt` not `lastUpdated`); `strategies.tsx` model-assumptions disclosure card; `/api/market/macroHistory` endpoint + 5D sparklines on VIX / DXY / India VIX / WTI Crude tiles in `<GlobalCuesStrip>`.
