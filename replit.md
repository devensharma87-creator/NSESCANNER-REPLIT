...
- **Indices Tab**: Dedicated dashboard at `/indices` (placed in nav right after `/scanner`) covering 14 instruments — Indian indices (NIFTY 50, BANK NIFTY, FIN NIFTY, MIDCAP NIFTY, SENSEX), global benchmarks (S&P 500, NASDAQ, Dow Jones, FTSE 100, Nikkei 225) and commodities (Gold, Silver, WTI Crude, Brent). Per instrument: live LTP / OHLC / change & %, 52w high/low, prev day OHLC, daily-close EMAs (9/20/50/100/200), session VWAP, market profile (VAH/VAL/POC) from a 24-bin intraday volume distribution, and floor-trader pivots (P, R1-R3, S1-S3) from prev day OHLC. Indian-index LTPs prefer the live Kite session when authenticated; otherwise everything falls back to Yahoo (~15 min delayed) with a visible "delayed" pill. Backend lib `indicesBoard.ts` caches the full snapshot for 10 s, fans out daily + intraday Yahoo fetches per instrument in parallel, and surfaces partial-data notes when a chart fetch returns insufficient bars (no synthetic placeholders). UI uses three filter chips (India / Global / Commodities), a compact table view (Name · LTP · %ΔΔ · VWAP · EMA20) with click-to-expand cards showing the full fact pack, and visual cues: green/red change colours, LTP underline coloured by VWAP relation, EMA cell outlines coloured by price-vs-EMA, and S/R bands shading when LTP is within 0.4% of any pivot level.
