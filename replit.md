...
- **Pass-1 Safety Nets** (2026-05-07): Five additive gates — no dial changes, no behaviour change to existing trades.
    ...
    5. **Deferred to Pass 2**: F&O invalid-spot-stop-geometry rejection (touches `optionSignals.ts` emission pipeline; the existing 30% premium-loss clamp is a safety, not a footgun). Also deferred per owner direction: dial changes (MIN_FNO_TRADE 70, halved per-trade risk, MAX_TRADES_PER_DAY 3); T1 partial booking; equity risk-based sizing; portfolio heat; market-regime filter; post-stop multiplier; global risk manager.
