...
- **Learn Tab Expansion**: Expanded content on futures, options, derivatives, trading psychology, and risk management. Topic 02 "Technical Analysis" rebuilt as a deep dive (Dow theory through Wyckoff, Volume Profile, ATR, ADX, anchored VWAP, Bulkowski pattern stats — 29 keyConcepts, 7 callouts, 10 video sources, 17 resources). User-facing free-form copy keeps the no-tool-name discipline; specific platform names appear only in clearly-typed Resource entries (matching the existing file convention). Topic 10 "Trading Psychology — Deep Dive" extended with 16 additional keyConcepts (Steenbarger Performance Pyramid, Tendler Inchworm + 7 tilt subtypes, cortisol/dopamine cycle, variance/drawdown math, mathematical risk-of-ruin, the 1% rule, identity-vs-behaviour, Stoic dichotomy of control, flow state, pre-mortem, 21/90 rule, decision fatigue, imposter syndrome, comparison trap, boredom).
