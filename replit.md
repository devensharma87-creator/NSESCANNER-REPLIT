...
- **Scanner Fit-to-Viewport**: The full NSE table (19 columns) was previously fixed at `width: 1426 px` which forced horizontal scrolling on 1280 px viewports and left empty space on wider monitors. Tightened all numeric column widths so the total budget = ~1322 px, switched the outer + header + body containers from `width: TOTAL_WIDTH` to `className="w-full"` + `minWidth: TOTAL_WIDTH`, and absolute-positioned rows to `left-0 right-0` so they stretch with the container. Both the **header** SCORE cell *and* every **data-row** SCORE cell use `className="px-2 flex-1"` + `style={{ minWidth: COL_WIDTHS.score }}`, so on viewports wider than the column total the SCORE column expands and absorbs the slack — there is never an empty band between the row's score bar and the SIGNAL pill. The SIGNAL cell carries `shrink-0` so it never compresses. Verified end-to-end: at 1280 px and 1500 px, `headerRow.lastChild.right` and `dataRow.lastChild.right` are equal within 2 px.
...
- **Pre/Post-Market Trader-Grade Additions**: The `/premarket` page now renders five additional sections backed by `getPreMarketReport()` in `lib/preMarket.ts`. (1) **Today's Game Plan**: three trading scenarios (BULLISH / BEARISH / RANGE) anchored to the live NIFTY 50 R1/S1 + CPR levels, with HIGH/MEDIUM/LOW probability scored from `sentimentScore` (cues scale -100..+100) plus GIFT NIFTY direction plus India VIX regime (calm <13 / elevated >18 / panic >22) — VIX explicitly weights the probabilities, not just narrative text. (2) **Key Index Levels** for the five active F&O indices (NIFTY 50, BANK NIFTY, FINNIFTY, NIFTY MIDCAP 50 via `^NSEMDCP50`, SENSEX via `^BSESN`): classic pivots (P/R1/R2/S1/S2) + Frank Ochoa CPR (TC/Pivot/BC + NARROW/NORMAL/WIDE width%) + 5d/22d/252d high-low bands + position-in-52w-range%, all computed from a 1-year daily fetch via `fetchChart(sym, "1y", "1d")` (the older `fetchIndexChart` only pulls 5d and is insufficient). Bands use the actual high/low arrays — close-only would systematically underestimate true intraday extremes. ^CNXNXT50 / NIFTYNXT50 has no working Yahoo symbol in 2026 so SENSEX takes the slot (Next 50 has no F&O lot anyway). (3) **Option Snapshot** for NIFTY/BANKNIFTY/FINNIFTY: ATM straddle, expected-move%, PCR (OI + volume), max-pain, max-call/max-put OI strike, and bias. Empty when the NSE option chain is geo-blocked AND no Kite session is active — the frontend hides the section silently. (4) **Sector Heatmap**: full ranking of all `SECTORS` sorted by avgChangePercent desc with gainers/losers/topPick per tile. (5) **FII / DII Snapshot**: latest day's FII + DII cash and 5-day cumulative. The orchestrator (`getPreMarketReport`) wraps every builder in `Promise.all` with an explicit per-builder `.catch` returning a shape-compatible empty fallback so a single upstream failure (Yahoo rate-limit, NSE CSV path change, Kite session expired, geo-block) can never short-circuit the response and 500 the whole report.
