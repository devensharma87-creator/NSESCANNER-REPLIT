...
- **F&O paper-trade data-quality gate** (`isActionableForFno` in `tradingConfig.ts`): `LIVE_KITE_*` always actionable; `STALE` always rejected; `DELAYED_YAHOO` is **actionable by default** (set env `PAPER_TRADE_KITE_ONLY=1` to restore strict Kite-only). Default flipped after a Kite-outage day produced "many triggered, zero executed" with no audit trail. Every skip is now recorded in the in-memory `MissedSignals` ring buffer (also covers `CONFIDENCE_FLOOR` and `MISSED_WINDOW`) and surfaced in the Paper Trading "Skipped & missed signals" card with a `skipReason` badge + `dataQuality` tooltip.
...
- **Outstanding audit backlog (next session)**: (1) `DataSourceBadge` placement on remaining low-traffic pages (`sectors`, `news`, `deep-scan`, `option-chain`, `dashboard` page-level header). Currently placed on: scanner, options, oi-lab, flows, premarket, stocks-to-watch. (2) Blank-panel diagnosis on `deep-scan`, `flows`, `options` charts (AUD-007/008, T009 — needs fresh-eyes investigation). (3) Indices Board "Loading…" stuck-state diagnosis (T008 server-side; UI-side market-mood guard already shipped). (4) 5-day mini-sparklines for VIX/DXY/Crude (T011 — needs new `/api/market/macroHistory` endpoint). (5) Yahoo-fallback gating for F&O Top-50 (T012, server-side). (6) 15-second opening-window polling for F&O Top-50 (T013). (7) FVG / Liquidity Sweep / Volume-Δ engines (T014–T016, blocked by T012). All five SMC items must ship behind feature flags so any single one can be disabled by env var.
