...
- **(A) Win-rate denominator fix** — `loadSetupWinRates` (`optionSignalGates.ts`) now uses an explicit *whitelist* of decided outcomes: `TARGET1_HIT / TARGET2_HIT / STOPPED / MANUAL_OVERRIDE` always count; `EXPIRED` counts only if `realized_pnl != 0`. Filled flat-EXPIRED rows (EOD sweep rescues) drop out. `paper_trade_fo` only contains filled trades by construction (insert is inside the open-txn after account debit), so no separate fill-state filter is needed. Pure-function classifier `isCountedForWinRate` extracted to `lib/winRateClassification.ts` for unit-testing.
