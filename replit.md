...
- `artifacts/api-server/src/lib/equitySizingHelper.ts`: pure-function equity sizing preview (Priority 5, 2026-05-15). `computeEquitySizingPreview(input)` mirrors `openPaperEquityTrade`'s gate sequence EXACTLY — INVALID_STOP → STOP_SANITY → DD_DAILY → DD_WEEKLY → DD_MONTHLY → DAILY_CAP → CONCURRENT_CAP → DEPLOY_LE_0 → QTY_LT_1 → INSUFF_BAL → HEAT_CAP — and returns `{verdict, reason, detail, qty, capitalRequired, perShareRisk, totalRisk, riskPct, accountValue, slots, perPosition, deploy, newHeat, projectedHeat, heatCap, limits}`. Constants (SEED_CAPITAL.EQUITY=1M, EQUITY_RISK.BASE_SLOTS=4 / MAX_CONCURRENT=10 / MAX_NEW_PER_DAY=3, EQUITY_STOP_SANITY 1-8%, PORTFOLIO_HEAT.MAX_EQ_HEAT_PCT=6%) single-sourced from `paperAccount.ts` and echoed in `result.limits` (drift-guarded by test). Owner-only diagnostics in `routes/equitySizing.ts`: `GET /api/paper/eq/sizing-preview?symbol=&entry=&stop=` (single-shot preview against a live account snapshot — calls `ensureDailyReset("EQUITY")` + reads sticky DD latches via `getEqDaily/Weekly/MonthlyRealizedDrawdown` for full live-path parity) and `GET /api/paper/eq/candidates-diagnostic` (batch preview over latest swing-scan candidates with `action <> 'AVOID / NO TRADE'`, plus reason histogram). **Read-only / preview only — never called from any execution path.** Both routes use strict-owner gate (no public-mode bypass).
- `lib/db/src/schema/candleWarehouse.ts`: durable OHLCV+OI candle warehouse (Priority 4, 2026-05-15). Two tables: `candle` (composite PK `instrument_token,interval,ts`; OHLC numeric, volume bigint, oi bigint nullable, source varchar, denormalised symbol/exchange tags) + `candle_sync_run` (per-cycle bookkeeping with kind/interval/universe/symbols/rows/errors). Ingestor `artifacts/api-server/src/lib/candleWarehouseIngestor.ts` reuses `fetchKiteHistoricalByToken` (already throttled at ~2.5 req/s with in-flight dedup) — no new broker calls. Universes via env CSV `CANDLE_WAREHOUSE_UNIVERSES` (default `indices`; allowed `indices,fno-stocks,swing-500`). Indices = NIFTY/BANKNIFTY/SENSEX (mirrors snapshot universe + `FNO_INDICES`). Intervals: `day` + `15minute` (5m intentionally excluded as too heavy for v1). Daily sync after 15:40 IST latched per IST-day; 15-min intraday loop during market hours; per-cycle symbol cap (`CANDLE_WAREHOUSE_MAX_SYMBOLS_PER_RUN`, default 60) so heavy universes spread across cycles. Backfill: 400-day daily / 30-day intraday on first sight; auto-detect BACKFILL vs INCREMENTAL by gap from latest stored ts (5-day threshold for daily, 1-day for 15m). Idempotent ON CONFLICT upsert on PK. Retention: intraday swept by `CANDLE_WAREHOUSE_RETENTION_DAYS_INTRADAY` (default 60); daily candles kept indefinitely. **Write-only data substrate — does NOT feed swing scanner / F&O signals / paper-trader / scoring / strategy builder / scanner / order paths.** Swing scanner's `fetchDailyBars` continues to read directly from `fetchKiteHistoricalByToken` (cache-fronted), NOT from this table. Env-gated by `CANDLE_WAREHOUSE_ENABLED` (auto-off in dev via `REPLIT_DEPLOYMENT` check; mirrors `isPaperAutoTradingEnabled`). Owner-only diagnostics: `GET /api/candles/diagnostics` (config + by-interval coverage + 100 most-stale per-symbol rows + recent 20 runs + in-memory ring buffer) and `POST /api/candles/sync?interval=&universe=&kind=&ignoreCap=1` (manual trigger). Both routes use a strict owner gate — they do NOT inherit the public-mode read bypass.
- `lib/db/src/schema/optionChainSnapshot.ts`: durable per-contract option snapshots (Priority 3, 2026-05-15). Two tables: `option_chain_snapshot` (composite PK `underlying,expiry,strike,opt_type,captured_at`; LTP/OI/ChgOI/volume/IV/bid/ask/Greeks/spread/depth nullable; `tradingsymbol`/`instrument_token` reserved nullable) + `option_chain_snapshot_run` (per-cycle bookkeeping). Ingestor `artifacts/api-server/src/lib/optionChainSnapshotIngestor.ts` runs every `OPTION_SNAPSHOT_INTERVAL_MIN` (default 5) during market hours over `SNAPSHOT_INDICES = ["NIFTY","BANKNIFTY","SENSEX"]` (mirrors `FNO_INDICES`), front+next expiry, ATM ± `OPTION_SNAPSHOT_STRIKE_WINDOW` (default 10) strikes, idempotent ON CONFLICT upsert. **Write-only data layer — does NOT feed any trading decision.** Env-gated by `OPTION_SNAPSHOT_ENABLED` (auto-off in dev via `REPLIT_DEPLOYMENT` check, mirrors `isPaperAutoTradingEnabled`). Daily retention sweep at `OPTION_SNAPSHOT_RETENTION_DAYS` (default 30). Owner-only diagnostics: `GET /api/option-snapshots/diagnostics` (config + per-underlying coverage + recent runs) and `POST /api/option-snapshots/run-now?force=1` (manual cycle, force bypasses market-hours guard). Both routes use a strict owner gate — they do NOT inherit the public-mode read bypass.
...
- **Kite session encryption-at-rest (2026-05-15)**: `kite_session.{api_key, access_token, public_token}` are encrypted at rest with AES-256-GCM (envelope `v1:<iv>:<tag>:<ct>`) when `KITE_TOKEN_ENC_KEY` is set (32 bytes as base64 or 64 hex chars). Implementation in `lib/kiteCrypto.ts`; wired through `kiteAuth.ts` (`completeLogin` / `storeImportedSession` encrypt-on-write, `getActiveSession` decrypts + lazy-migrates plaintext rows on read). If `KITE_TOKEN_ENC_KEY` is unset, the code falls back to plaintext storage with a one-time warn so deploys without the secret don't break — **always set the key in production**. Decrypt failures are fail-closed (`getActiveSession` returns `null`, daily login flow recovers). The token self-rotates daily at ~06:00 IST regardless. **Never share a raw `pg_dump` of the live DB** — even encrypted at rest, the API_SECRET in env can re-mint sessions. Use **`scripts/safe-db-export.sh`** (excludes `kite_session` entirely + scrubs `users.password_hash` + `global_screener_presets.share_token`) for any dump that leaves the server.
